...
select * from pedido_pedidows where entregado=true
...
select pws.id,case when pws.num_pedido is null or length(pws.num_pedido) =0 then 'PdWs' else pws.num_pedido end as nump,
	...
	0 as supervisor_id,
	'Alistador Pws' as alistador,
	m.first_name||' '||m.last_name as motori,
	from pedido_pedidows as pws inner join auth_user as m ON (pws.motorizado_id = m.id)
	where pws.entregado=true
