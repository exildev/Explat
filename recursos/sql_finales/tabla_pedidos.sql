-- Function: tabla_pedidos(integer, text, text, integer, integer)

-- DROP FUNCTION tabla_pedidos(integer, text, text, integer, integer);

CREATE OR REPLACE FUNCTION tabla_pedidos(id_des integer, search_ text, order_ text, start_ integer, length_ integer)
  RETURNS text AS
$BODY$
declare
	l json;
...
begin
	...
	SELECT COALESCE(array_to_json(array_agg(row_to_json(p))), '[]') from (
		select case when p.activado then 1 else 0 end as activado,p.num_pedido as num,emp.first_name as emp,a3.first_name||' '||a3.last_name as sup,u1.first_name||' '||u1.last_name as alis,
		u2.first_name||' '||u2.last_name as moto,case when p.total is not null then to_char(p.total,'LFM9,999,999,00') else to_char(0,'LFM9,999,999.00') end as total,
		...
		or u1.last_name like '%'||search_||'%' or u2.first_name like '%'||search_||'%' or u2.last_name like '%'||search_||'%'  order by p.fecha_pedido desc limit length_ offset start_
	) p into l;
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION tabla_pedidos(integer, text, text, integer, integer)
  OWNER TO postgres;
